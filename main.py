...
st.set_page_config(page_title="Loan Default Prediction - Logistic Regression", layout="wide")
...
# Streamlit UI
st.title("Loan Default Prediction (Logistic Regression)")
st.subheader("Model Performance")

perf_df = pd.DataFrame(metrics, index=["Logistic Regression"]).T
perf_df.index.name = 'Metric'
st.table(perf_df)

st.subheader("Predict Applicant's Default Risk")
st.write("Fill in the applicant's details below:")

with st.form('input_form'):
    inputs = {}
    col1, col2 = st.columns(2)
    all_features = numeric_cols + cat_cols
    col1_fields = all_features[:len(all_features)//2]
    col2_fields = all_features[len(all_features)//2:]
